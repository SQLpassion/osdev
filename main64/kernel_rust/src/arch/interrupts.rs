...
fn init_idt() {
    unsafe {
        ...
        idt[IRQ0_VECTOR as usize].set_handler(irq0_pit_timer_stub as *const () as usize);
        idt[IRQ1_VECTOR as usize].set_handler(irq1_keyboard_stub as *const () as usize);
        idt[IRQ2_VECTOR as usize].set_handler(irq2_pic_cascade_stub as *const () as usize);
        idt[IRQ3_VECTOR as usize].set_handler(irq3_com2_stub as *const () as usize);
        idt[IRQ4_VECTOR as usize].set_handler(irq4_com1_stub as *const () as usize);
        idt[IRQ5_VECTOR as usize].set_handler(irq5_lpt2_or_sound_stub as *const () as usize);
        idt[IRQ6_VECTOR as usize].set_handler(irq6_floppy_stub as *const () as usize);
        idt[IRQ7_VECTOR as usize].set_handler(irq7_lpt1_or_spurious_stub as *const () as usize);
        idt[IRQ8_VECTOR as usize].set_handler(irq8_cmos_rtc_stub as *const () as usize);
        idt[IRQ9_VECTOR as usize].set_handler(irq9_acpi_or_legacy_stub as *const () as usize);
        idt[IRQ10_VECTOR as usize].set_handler(irq10_free_stub as *const () as usize);
        idt[IRQ11_VECTOR as usize].set_handler(irq11_free_stub as *const () as usize);
        idt[IRQ12_VECTOR as usize].set_handler(irq12_ps2_mouse_stub as *const () as usize);
        idt[IRQ13_VECTOR as usize].set_handler(irq13_fpu_stub as *const () as usize);
        idt[IRQ14_VECTOR as usize].set_handler(irq14_primary_ata_stub as *const () as usize);
        idt[IRQ15_VECTOR as usize].set_handler(irq15_secondary_ata_stub as *const () as usize);
