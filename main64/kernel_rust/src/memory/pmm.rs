...
impl PhysicalMemoryManager {
    ...
    /// Runs a basic PMM allocation/release self-test and prints results.
    pub fn test(&mut self, screen: &mut Screen) {
        let mut first = [None, None, None];

        for slot in first.iter_mut() {
            *slot = self.alloc_frame();
        }

        if first.iter().any(|pf| pf.is_none()) {
            writeln!(screen, "[pmm] test: failed (initial alloc)").unwrap();
            for pf in first.into_iter().flatten() {
                self.release_frame(pf);
            }
            return;
        }

        let f0_pfn = first[0].as_ref().unwrap().pfn;
        let f1_pfn = first[1].as_ref().unwrap().pfn;
        let f2_pfn = first[2].as_ref().unwrap().pfn;
        ...
        // Release the middle frame to check reuse.
        if let Some(pf) = first[1].take() {
            writeln!(screen, "[pmm] release: pfn={}", pf.pfn).unwrap();
            self.release_frame(pf);
        } else {
            writeln!(screen, "[pmm] release: failed (missing frame)").unwrap();
        }

        let mut second = [None, None, None];
        for slot in second.iter_mut() {
            *slot = self.alloc_frame();
        }

        if second.iter().any(|pf| pf.is_none()) {
            writeln!(screen, "[pmm] test: failed (realloc)").unwrap();
            for pf in second.into_iter().flatten() {
                self.release_frame(pf);
            }
            if let Some(pf) = first[0].take() {
                self.release_frame(pf);
            }
            if let Some(pf) = first[2].take() {
                self.release_frame(pf);
            }
            return;
        }

        let s0_pfn = second[0].as_ref().unwrap().pfn;
        let s1_pfn = second[1].as_ref().unwrap().pfn;
        let s2_pfn = second[2].as_ref().unwrap().pfn;
        ...
        let reused = s0_pfn == f1_pfn;
        let new_pfns = s1_pfn > f2_pfn && s2_pfn > s1_pfn && s1_pfn != f0_pfn;

        if reused && new_pfns {
            writeln!(screen, "[pmm] test: ok").unwrap();
        } else {
            writeln!(
                screen,
                "[pmm] test: failed (reuse={}, new={})",
                reused,
                new_pfns
            )
            .unwrap();
        ...
        for pf in second.iter_mut().filter_map(Option::take) {
            self.release_frame(pf);
        }
        self.release_frame(first[0].take().unwrap());
        self.release_frame(first[2].take().unwrap());
    }
}
