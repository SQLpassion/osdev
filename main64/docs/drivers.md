...
`UNLOAD_DRIVER` is a later addition to this same set, and its purpose is narrower and more specific than `MMIO`: it gates `DrvUnload` (§13), and it is never handed to a spawned driver itself — only to `DRIVERS.BIN`, by delegation, at `Exec` time (§21). Enumerating the driver registry via `DrvList` (§20) is not gated by any capability at all — resolving driver names/task-ids is not itself a privileged operation, so any task, including one with zero capabilities, may call it. `Capabilities::from_bits_truncate()` masks out any bit the caller didn't legitimately request, and a second mask, `driver_db::DRIVER_GRANTABLE_CAPS`, further restricts what `SpawnDriver` itself will ever attach to a *driver* task — that mask is exactly `MMIO`, so a driver process can never end up holding `SPAWN_DRIVER` or `UNLOAD_DRIVER` no matter what it asks for, closing off any possibility of a compromised driver spawning further drivers or unloading its siblings.
...
Deciding *which* binary gets *which* capabilities is deliberately kept out of the kernel. The kernel only provides the delegation mechanism; the policy of "only `DRIVERS.BIN`, and only these two bits" lives entirely in the shell's `run_program()`, in a function just as small and just as directly tested:
