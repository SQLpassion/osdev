...
Floppy disk formatting configurations (like those executed during the generation of the bootable image `kaos64.img` via the build tool `fat_imgen`) rely on the layout properties specified in the BIOS Parameter Block (BPB) located at physical sector 0 (LBA 0).
