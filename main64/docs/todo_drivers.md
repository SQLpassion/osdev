...
> ELF directly, so the script ships the (stripped) ELF binary as-is. Drivers
> built the same way as other user programs will ship as ELF too; this does
> not change anything else in this plan (§2's "no ELF loader needed" argument
> is about *dynamic linking/relocation*, which the static `ET_EXEC` loader
