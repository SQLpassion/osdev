...
pub fn find_esp_start_lba() -> Option<u64> {
    ...
    let sectors_to_read = (max_entries_to_check + entries_per_sector - 1) / entries_per_sector;
    ...
    for sector_offset in 0..sectors_to_read {
        ...
        if let Some(start_lba) = parse_gpt_entries_sector(&entry_sector, entries_in_this_sector, entry_size) {
...
pub fn parse_gpt_entries_sector(
    ...
    for i in 0..entries_in_this_sector {
        ...
        if guid == ESP_TYPE_GUID {
            let start_lba = u64::from_le_bytes(entry_sector[offset + 0x20..offset + 0x28].try_into().unwrap());
            return Some(start_lba);
        }
    }
    None
