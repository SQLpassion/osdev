//! The shell's `load <name.drv>` command (Phase 2 Step 7 of
//! `docs/nic_driver_design.md` §4.2-4.3): spawns a NIC driver as a
//! background process, without blocking the shell on `process::wait`.
//!
...
/// the driver runs independently and the shell prompt returns immediately).
///
...
pub fn load_driver(file: &str) {
    ...
    let canonical_name = match resolve_driver_filename(file, DRIVER_TABLE, &devices) {
        Ok(name) => name,
        Err(ResolveError::UnknownDriver) => {
            println!("[shell] Unknown driver '{}'.", file);
            return;
        }
        Err(ResolveError::DeviceNotPresent) => {
            println!(
                "[shell] Error: no matching PCI device found for '{}'.",
                file
            );
            return;
        }
    ...
    // Step 5: spawn in the background -- no process::wait() call. This is
    // the behavioral change from the removed run_rtl8139_driver/
    // run_intel_nic_driver, which blocked the shell until the driver exited.
    match lib_driver::spawn::spawn_driver(canonical_name, caps, None) {
        Ok(tid) => {
            println!("[shell] Driver '{}' started as TID {}", canonical_name, tid);
        }
        Err(err) => {
            println!("[shell] Failed to load '{}': {:?}", canonical_name, err);
        }
    }
}
